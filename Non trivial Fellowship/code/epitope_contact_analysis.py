"""
epitope_contact_analysis.py

Heavy-atom, clash-floor-corrected contact analysis for scoring predicted
binder:target complexes against a defined epitope.

WHY THE CLASH FLOOR MATTERS
---------------------------
An earlier version of this analysis (used during this project's RF2
pre-filter stage) counted ALL atoms -- including hydrogens -- with a simple
nearest-atom distance cutoff and no lower bound. This counted physically
IMPOSSIBLE sub-1-Angstrom atom coincidences (hydrogen-hydrogen near-overlaps,
in one case 0.15 Angstrom) as genuine contacts, which inflated reported
hotspot engagement (e.g. from a true max of 3/5 hotspots to a false "5/5").
Two changes fix this:
  1. Use heavy atoms only (skip any atom name starting with H, or a name
     like "1HG"/"2HD1" where a leading digit precedes the H).
  2. Apply a lower distance bound (CLASH_FLOOR, default 1.5 Angstrom) in
     addition to the usual upper cutoff (CONTACT_CUTOFF, default 4.5
     Angstrom) -- a "contact" closer than the clash floor is a modeling
     artifact, not a real interaction.

This module also reports clash diagnostics (overall_min_heavy_atom_dist,
n_severe_clash_pairs) so that a structure passing on contacts can still be
flagged if its interface geometry is otherwise implausible -- useful for
RF2/AF2 output, which selects on pLDDT alone and does not itself validate
physical geometry.

Usage
-----
    from epitope_contact_analysis import analyze_complex

    result = analyze_complex(
        pdb_path="complex.pdb",
        binder_chain="A", target_chain="B",
        epitope_range=range(261, 306),
        hotspots=[269, 272, 274, 294, 296],
    )
    print(result["n_epitope_contacts"], result["hotspots_engaged"])
"""
