"""
denv_conservation_check.py

Pan-serotype (DENV1-4) conservation check for NS1 epitope residues.

IMPORTANT LESSON FROM THIS PROJECT: checking whether a set of positions is
"conserved" only among a set of REFERENCE sequences is not sufficient --
you must also confirm that the ACTUAL modeling target sequence (the one used
in your structure predictions) matches those references at the positions
you care about. In this project, an initial check found all 5 designed
hotspots conserved among the DENV1-4 reference FASTAs, but a follow-up review
caught that the actual Boltz-2 modeling target diverged from all four
references at one hotspot (native position 272: R in the modeling target,
K in all four DENV1-4 references) -- a real, if conservative, mismatch that
the reference-only check would never surface. Always run check_target_vs_references
in addition to check_reference_conservation.

Usage
-----
    from denv_conservation_check import load_fasta, check_reference_conservation, check_target_vs_references

    serotypes = load_fasta("ns1_serotypes.fasta")   # {"DENV1": seq, "DENV2": seq, ...}
    ref_report = check_reference_conservation(serotypes, positions=[269,272,274,294,296])
    target_report = check_target_vs_references(serotypes, target_seq=MY_MODELING_TARGET,
                                                positions=[269,272,274,294,296])
"""
